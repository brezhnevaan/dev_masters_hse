...
Request:
{
    ...
    "password": "password123",
     "is_active": true,         # необязательное автозаполняемое поле
    "is_superuser": false,      # необязательное автозаполняемое поле
    "is_verified": false        # необязательное автозаполняемое поле
}
}
...
Request:
{
    ...
    "custom_alias": "string",               # необязательное поле
    "expires_at": "2025-12-31 23:59"        # необязательное поле
}
...
Response:
{
  ...
  "original_url": "https://example.com",
  "short_code": "string",                   # при незаполненном request: уникальный сгененированный алиас
  "expires_at": "2025-12-31 23:59"          # при незаполненном request: автоматический срок жизни +14 дней к дате-времени создания, обновляется при переходе по shortcode на +14 дней с даты-времени последнего клика
}
```
...
# List всех алиасов для искомого url, принадлежащих пользователю
...
{
    "short_code": "old-code",
    "new_short_code": "new-code"            # необязательное поле
}
...
Response:
{
  ...
  "original_url": "https://example.com",
  "short_code": "new-code"                  # при незаполненном request: уникальный сгененированный алиас
}
```
...
{
  "id": 0,
  "new_custom_alias": "string",                     # необязательное поле, проверяется уникальность текущего алиаса, если не занят — оставляет старый, если занят — гененирует автоматический уникальный код
  "new_expires_at": "2025-04-01T16:54:09.073Z"      # необязательное поле, создает новый срок жизни по правилу + 14 дней к моменту активации
}
